...
EGLBoolean EglContext::init(EGLContext sharedContext /* = nullptr */,
  ...
  AV_LOGD_ID(LOG_TAG, _sessionID, "[%s:%d] [EGL] eglInitialize %d.%d .\n",
  ...
  if (window) {
    ...
    _surface = eglCreateWindowSurface(_display, config, window, surfaceAttribs);
...
void EglContext::EGLTerminate() {
  ...
  if (_display) {
    ...
    AV_LOGI_ID(LOG_TAG, _sessionID,
               "[%s:%d] currentDisplay:%p, currentDrawSurface:%p, "
               "currentReadSurface:%p, currentContext:%p.\n",
...
EGLBoolean EglContext::createEGLSurfaceWithWindow(EGLNativeWindowType window) {
  std::unique_lock<std::mutex> lck(_eglMutex);
  ...
  if (!_surface) {
    EGLint surfaceAttribs[] = {EGL_NONE};
    _surface =
        eglCreateWindowSurface(_display, _config, window, surfaceAttribs);
    if (!_surface) {
      AV_LOGE_ID(
          LOG_TAG, _sessionID,
          "[%s:%d] [EGL] eglCreateWindowSurface failed , error=0x%04X .\n",
...
EGLBoolean EglContext::setSurfaceSizeWithWindow() {
...
#ifdef __ANDROID__
  {
    ...
    int ret = ANativeWindow_setBuffersGeometry(_window, width, height,
                                               native_visual_id);
    if (ret) {
      AV_LOGE_ID(LOG_TAG, _sessionID,
...
EGLBoolean EglContext::setSurfaceSize(int width, int height) {
  ...
  if (width != _width || height != _height) {
    ...
    if (ret) {
      AV_LOGE_ID(LOG_TAG, _sessionID,
                 "[%s:%d] ANativeWindow_setBuffersGeometry error:%d .\n",
                 __FUNCTION__, __LINE__, ret);
      return EGL_FALSE;
    }
