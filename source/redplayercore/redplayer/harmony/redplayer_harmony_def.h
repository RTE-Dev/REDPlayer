...
public:
  explicit RedNativeWindow(OHNativeWindow *native_window) {
    AV_LOGD("RedNativeWindow", "RedNativeWindow ctor %p\n", native_window);
    if (native_window) {
      AV_LOGD("RedNativeWindow", "ANativeWindow_acquire window %p\n",
              native_window);
  ...
  ~RedNativeWindow() {
    AV_LOGD("RedNativeWindow", "~RedNativeWindow dtor %p\n", native_window_);
    if (native_window_) {
      AV_LOGD("RedNativeWindow", "ANativeWindow_release window %p\n",
              native_window_);
      //    ANativeWindow_release(native_window_);
      native_window_ = nullptr;
    }
  }
  OHNativeWindow *get() { return native_window_; }
