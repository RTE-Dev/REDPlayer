...
- (instancetype)init {
    ...
    if (self) {
        ...
        _serialQueue = dispatch_queue_create("com.xiaohongshu.red.preload.queue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)openJson:(NSString *)jsonStr param:(RedPreloadParam)param userData:(void *)userData {
    ...
    dispatch_async([RedPreLoad shared].serialQueue, ^{
        ...
        // 获取缓存的档位url
        std::unique_ptr<redstrategycenter::strategy::RedAdaptiveStrategy> strategy = nullptr;
        strategy = std::make_unique<redstrategycenter::strategy::RedAdaptiveStrategy>(redstrategycenter::adaptive::logic::AbstractAdaptationLogic::LogicType::Adaptive);
        if (strategy) {
            strategy->setPlaylist(url);
            strategy->setReddownloadUrlCachedFunc(reddownload_datasource_wrapper_cache_size);
            url = strategy->getInitialUrl(strategy->getInitialRepresentation());
        }
        if (url.empty()) {
            av_log(NULL, AV_LOG_ERROR, "preload open json %s failed", jsonStr.UTF8String);
        }
        self.cacheUrlString = [NSString stringWithCString:url.c_str() encoding:NSUTF8StringEncoding];
...
- (void)open:(NSURL *)url param:(RedPreloadParam)param userData:(void *)userData {
...
+ (BOOL)isPreloadUrlHaveCached:(NSString *)url {
    if (!url)
        return false;
    int64_t cache_size = reddownload_datasource_wrapper_cache_size(url.UTF8String);
...
+ (int64_t)getCachedSize:(NSString *)path uri:(NSString *)uri isFullUrl:(BOOL)isFullUrl {
    if (!path || !uri)
        return -1;
    int64_t cache_size = reddownload_datasource_wrapper_cache_size_by_uri(path.UTF8String, uri.UTF8String, isFullUrl);
    av_log(NULL, AV_LOG_INFO, "preload get cache size:%" PRId64 " path: %s url: %s\n", cache_size, path.UTF8String, uri.UTF8String);
    return cache_size;
}
...
+ (NSArray<NSString *> *)getAllCachedFile:(NSString *)path {
    ...
    reddownload_get_all_cached_file(path.UTF8String, &cached_file, &cached_file_len);
...
+ (NSString *)getCacheFilePath:(NSString *)cachePath url:(NSString *)url {
    ...
    int ret = reddownload_get_cache_file_path(cachePath.UTF8String, url.UTF8String, &cache_file_path);
...
static void appcb(void *app_ctx, int msg, void *arg1, void *arg2) {
    ...
    switch (msg) {
        case RED_EVENT_DID_FRAGMENT_COMPLETE_W: {
            ...
            task.error = 0;
            task.url = [NSString stringWithCString:traffic->url encoding:NSUTF8StringEncoding];
        ...
        case RED_EVENT_DID_HTTP_OPEN_W: {
            ...
            if (httpEvent->error != 0) {
                ...
                task.url = [NSString stringWithCString:httpEvent->url encoding:NSUTF8StringEncoding];
                task.tcpSpeed = 0;
                task.trafficBytes = 0;
        ...
        case RED_CTRL_DID_TCP_OPEN_W: {
            ...
            if (tcpIO->error != 0) {
                ...
                task.url = [NSString stringWithCString:tcpIO->url encoding:NSUTF8StringEncoding];
                task.tcpSpeed = 0;
                task.trafficBytes = 0;
                block(task, RedPreLoadControllerMsgTypeError, app_ctx);
            }
...
static void speedcb(void *opaque, int64_t size, int64_t speed, int64_t cur_time) {
    redstrategycenter::RedStrategyCenter::GetInstance()->updateDownloadRate(size, speed, cur_time);
