...
#import <RedDownload/reddownload_datasource_wrapper.h>
...
- (instancetype)initWithContentURLString:(NSString *)aUrlString
    ...
    if (self) {
        ...
        _id = ++g_id;
        _mediaPlayer = CRedPlayer::Create(_id, std::bind(&media_player_msg_loop, std::placeholders::_1));
        ...
        _mediaPlayer->setConfig(cfgTypePlayer, "start-on-prepared", _shouldAutoplay ? 1 : 0);

        _syncQueue = dispatch_queue_create("com.xiaohongshu.player.red.sync.queue", DISPATCH_QUEUE_SERIAL);
        ...
        _glView = (UIView<RedRenderViewProtocol> *)_mediaPlayer->initWithFrame((int)renderType, cgrect);
...
+ (void)setLogCallback:(RedLogCallback)logCallback {
    [RedPlayerController setLogCallback:logCallback logCallScene:REDLogCallScene_Default];
}
...
+ (void)initreddownload:(NSString *)cachePath maxSize:(uint32_t)maxSize threadpoolSize:(int)poolSize {
    if (cachePath == nil) {
...
+ (void)initreddownload:(NSString *)cachePath maxSize:(uint32_t)maxSize threadpoolSize:(int)poolSize maxdircapacity:(int64_t)maxdircapacity {
    if (cachePath == nil) {
        AV_LOGE(TAG, "initreddownload cachePath is nil\n");
        return;
    }
...
+ (void)setreddownloadGlobalConfig:(NSDictionary<NSString *, NSNumber *> *)configDic {
...
- (void)shutdown {
    ...
    void (^releaseBlock)(void) = ^{
        ...
        __unused id weakHolder = (__bridge_transfer REDWeakHolder *)self->_mediaPlayer->setInjectOpaque(nullptr);
        ...
        void (^block)(void) = ^{
            ...
            __unused id weakPlayer = (__bridge_transfer RedPlayerController *)self->_mediaPlayer->setWeakThiz(nullptr);

            self->_mediaPlayer.reset();
            [self->_glView removeFromSuperview];
            [self->_glView shutdown];
            // AV_LOGD_ID(TAG, self->_id, "%s glView: didShutdown\n", __func__);
            self->_view = nil;
            self->_glView = nil;
            // AV_LOGD_ID(TAG, self->_id, "%s view: did dealloc\n", __func__);

            [self didShutdown];
            // AV_LOGD_ID(TAG, self->_id, "%s player: didShutdown\n", __func__);
        };
...
- (NSTimeInterval)playableDuration {
    ...
    video_cached_ms = _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_CACHED_DURATION, (int64_t)0);
    audio_cached_ms = _mediaPlayer->getProp(RED_PROP_INT64_AUDIO_CACHED_DURATION, (int64_t)0);
    _mediaPlayer->getCurrentPosition(pos);
...
- (NSDictionary *)getVideoDebugInfo {
    ...
    int64_t vdec = _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_DECODER, (int64_t)RED_PROPV_DECODER_UNKNOWN);
    float vdps = _mediaPlayer->getProp(RED_PROP_FLOAT_VIDEO_DECODE_FRAMES_PER_SECOND, .0f);
    float vfps = _mediaPlayer->getProp(RED_PROP_FLOAT_VIDEO_OUTPUT_FRAMES_PER_SECOND, .0f);
    int64_t pix_fmt = _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_PIXEL_FORMAT, (int64_t)AV_PIX_FMT_NONE);
    int64_t bit_rate = _mediaPlayer->getProp(RED_PROP_INT64_BIT_RATE, (int64_t)0);
    int64_t seek_cost = _mediaPlayer->getProp(RED_PROP_INT64_LATEST_SEEK_LOAD_DURATION, (int64_t)0);
    ...
    [debugInfo setValue:[NSString stringWithFormat:@"%.2f / %.2f", vdps, vfps] forKey:@"fps"];
    [debugInfo setValue:[NSString stringWithFormat:@"%.3d /%.3d", (int)_videoWidth, (int)_videoHeight] forKey:@"resolution"];
    [debugInfo setValue:[NSString stringWithFormat:@"%@/s", formatedSize(bit_rate / 8)] forKey:@"bitrate"];
    int64_t vcacheb = _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_CACHED_BYTES, (int64_t)0);
    int64_t acacheb = _mediaPlayer->getProp(RED_PROP_INT64_AUDIO_CACHED_BYTES, (int64_t)0);
    int64_t vcached = _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_CACHED_DURATION, (int64_t)0);
    int64_t acached = _mediaPlayer->getProp(RED_PROP_INT64_AUDIO_CACHED_DURATION, (int64_t)0);
    int64_t vcachep = _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_CACHED_PACKETS, (int64_t)0);
    int64_t acachep = _mediaPlayer->getProp(RED_PROP_INT64_AUDIO_CACHED_PACKETS, (int64_t)0);
    ...
    [debugInfo setValue:[NSString stringWithFormat:@"%.3f %.3f", avdelay, -avdiff] forKey:@"delay"];

    [debugInfo setValue:@(self.playbackRate)
                 forKey:@"speed"];
    [debugInfo setValue:@(self.playbackVolume)
                 forKey:@"volume"];
    [debugInfo setValue:formatedDurationMilli(seek_cost)
                 forKey:@"seek_cost"];
...
- (float)dropPacketRateBeforeDecode {
    if (!_mediaPlayer)
        return 0.0f;
    return _mediaPlayer->getProp(RED_PROP_FLOAT_DROP_PACKET_RATE_BEFORE_DECODE, 0.0f);
}
...
- (float)vdps {
    if (!_mediaPlayer)
        return 0.0f;
    return _mediaPlayer->getProp(RED_PROP_FLOAT_VIDEO_DECODE_FRAMES_PER_SECOND, 0.0f);
}

- (float)vRenderFps {
    if (!_mediaPlayer)
        return 0.0f;
    return _mediaPlayer->getProp(RED_PROP_FLOAT_VIDEO_OUTPUT_FRAMES_PER_SECOND, 0.0f);
}
...
- (int64_t)cachedDuration {
    ...
    return _mediaPlayer->getProp(RED_PROP_INT64_VIDEO_CACHED_DURATION, (int64_t)0);
}
...
- (int64_t)videoCachedBytes {
    ...
    return _mediaPlayer->getProp(RED_PROP_INT64_CACHED_STATISTIC_BYTE_COUNT, (int64_t)0);
}
...
- (int64_t)videoRealCachedBytes {
    ...
    return _mediaPlayer->getProp(RED_PROP_INT64_CACHED_STATISTIC_REAL_BYTE_COUNT, (int64_t)0);
}
...
static void customLogCallback(int level, const char *tagstr, const char *logstr) {
...
- (void)postEvent:(sp<Message>)msg {
    ...
    switch (msg->mWhat) {
        ...
        case RED_MSG_ERROR: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerPlaybackStateDidChangeNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];

            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerPlaybackDidFinishNotification
                                                                object:self
                                                              userInfo:@{RedPlayerPlaybackDidFinishReasonUserInfoKey : @(RedFinishReasonPlaybackError),
                                                                         RedPlayerPlaybackDidFinishErrorUserInfoKey : @(arg1),
                                                                         RedPlayerPlaybackDidFinishDetailedErrorUserInfoKey : @(arg2),
        ...
        case RED_MSG_PREPARED: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedMediaPlaybackIsPreparedToPlayDidChangeNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            _loadState = RedLoadStatePlayable | RedLoadStatePlaythroughOK;

            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerLoadStateDidChangeNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
        ...
        case RED_MSG_COMPLETED: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerPlaybackDidFinishNotification
                                                                object:self
                                                              userInfo:@{RedPlayerPlaybackDidFinishReasonUserInfoKey : @(RedFinishReasonPlaybackEnded),
                                                                         RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        ...
        case RED_MSG_SEEK_COMPLETE: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerDidSeekCompleteNotification
                                                                object:self
                                                              userInfo:@{RedPlayerDidSeekCompleteTargetKey : @(arg1),
        ...
        case RED_MSG_VIDEO_DECODER_OPEN: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerVideoDecoderOpenNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_VIDEO_RENDERING_START: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerFirstVideoFrameRenderedNotification
                                                                object:self
                                                              userInfo:@{RedPlayerFirstVideoFrameRenderedNotificationUserInfo : @(arg1),
                                                                         RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_AUDIO_RENDERING_START: {
            // AV_LOGD_ID(TAG, _id, "RED_MSG_AUDIO_RENDERING_START:\n");
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerFirstAudioFrameRenderedNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_AUDIO_DECODED_START: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerFirstAudioFrameDecodedNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_VIDEO_DECODED_START: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerFirstVideoFrameDecodedNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_OPEN_INPUT: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerOpenInputNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_FIND_STREAM_INFO: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerFindStreamInfoNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_COMPONENT_OPEN: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerComponentOpenNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_ACCURATE_SEEK_COMPLETE: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerAccurateSeekCompleteNotification
                                                                object:self
                                                              userInfo:@{RedPlayerDidAccurateSeekCompleteCurPos : @(arg1),
                                                                         RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_VIDEO_SEEK_RENDERING_START: {
            ...
            _isVideoSync = arg1;
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerSeekVideoStartNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            _isVideoSync = 0;
            break;
        ...
        case RED_MSG_URL_CHANGE: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerUrlChangeMsgNotification
                                                                object:self
                                                              userInfo:@{RedPlayerUrlChangeCurUrlKey : (current_url ?: @""),
        ...
        case RED_MSG_AUDIO_SEEK_RENDERING_START: {
            ...
            _isAudioSync = arg1;
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerSeekAudioStartNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            _isAudioSync = 0;
            break;
        ...
        case RED_MSG_VIDEO_FIRST_PACKET_IN_DECODER: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerVideoFirstPacketInDecoderNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_VIDEO_START_ON_PLAYING: {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerVideoStartOnPlayingNotification
                                                                object:self
                                                              userInfo:@{RedPlayerMessageTimeUserInfoKey : @(time)}];
            break;
        }
        case RED_MSG_VIDEO_ROTATION_CHANGED: {
            ...
            if (_rotateDegrees > 0) {
                ...
                _glView.transform = CGAffineTransformMakeRotation(M_PI * _rotateDegrees / 180.0);
...
int media_player_msg_loop(CRedPlayer* mp) {
    @autoreleasepool {
        __weak RedPlayerController *playerController = redPlayerRetain(mp->setWeakThiz(nullptr));
...
static int onInectREDIOStatistic(RedPlayerController *mpc, int type, void *data) {
    ...
    NSString *realUrl = [mpc->_urlString substringFromIndex:realUrlStartRange.location];
    NSURL *URL = [NSURL URLWithString:realUrl];
    if (URL && realData->cached_size == realData->logical_file_size) {
        [[NSNotificationCenter defaultCenter] postNotificationName:RedPlayerCacheDidFinishNotification
                                                            object:mpc
...
static bool red_selecturl_callback(void *opaque, const char *oldUrl, int reconnectType, char *outUrl, int outLen) {
    return false;
}
...
- (void)setMaxBufferSize:(int)maxBufferSize {
    if (_mediaPlayer) {
        _mediaPlayer->setConfig(cfgTypePlayer, "max-buffer-size", (int64_t)maxBufferSize);
    }
}
...
- (void)applicationWillEnterForeground {
    NSLog(@"RedPlayerController:applicationWillEnterForeground: %d", (int)[UIApplication sharedApplication].applicationState);
}

- (void)applicationDidBecomeActive {
    NSLog(@"RedPlayerController:applicationDidBecomeActive: %d", (int)[UIApplication sharedApplication].applicationState);
}

- (void)applicationWillResignActive {
    NSLog(@"RedPlayerController:applicationWillResignActive: %d", (int)[UIApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)applicationDidEnterBackground {
    NSLog(@"RedPlayerController:applicationDidEnterBackground: %d", (int)[UIApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_pauseInBackground) {
            [self pause];
        }
...
- (void)applicationWillTerminate {
    NSLog(@"RedPlayerController:applicationWillTerminate: %d", (int)[UIApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_pauseInBackground) {
            [self pause];
        }
