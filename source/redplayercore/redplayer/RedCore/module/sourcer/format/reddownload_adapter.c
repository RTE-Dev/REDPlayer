...
static void speedcallback(void* opaque, int64_t size, int64_t speed, int64_t cur_time) {
}
    ...
    if (app_ctx && app_ctx->func_on_app_event) {
        ...
        case RED_CTRL_WILL_TCP_OPEN_W: {
            ...
            if (a1) {
                ...
                app_ctx->func_on_app_event(app_ctx, REDIOAPP_EVENT_DEFAULT_TCP_RECV_BUFFER, arg1_w);
                free(arg1_w);
            }
            break;
        }
...
static int reddownload_adapter_open(URLContext *h, const char *arg, int flags, AVDictionary **options)
{
...
static int reddownload_adapter_read(URLContext *h, unsigned char *buf, int size)
{
    Context *c = h->priv_data;
...
static int64_t reddownload_adapter_seek(URLContext *h, int64_t pos, int whence)
{
    Context *c = h->priv_data;
    int64_t ret = reddownload_datasource_wrapper_seek(c->url, c->uid, pos, whence);
    return ret;
}
...
static const AVOption options[] = {
    { "cache_file_dir", "cache file dir", OFFSET(cache_file_dir), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "pre_download_size", "pre download size", OFFSET(pre_download_size), AV_OPT_TYPE_INT64, { .i64 = -1 }, -1, INT64_MAX, D },
    { "cache_max_dir_capacity", "cache max dir capacity", OFFSET(cache_max_dir_capacity), AV_OPT_TYPE_INT64, { .i64 = -1 }, -1, INT64_MAX, D },
    { "ip_type", "ip type", OFFSET(ip_type), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, .flags = D },
    { "dns_cache_timeout", "dns cache TTL (in microseconds)",   OFFSET(dns_cache_timeout), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
    { "headers", "headers", OFFSET(headers), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "useragent", "useragent", OFFSET(useragent), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "max_retry", "max retry", OFFSET(max_retry), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, .flags = D },
    { "low_speed_limit", "low speed limit", OFFSET(low_speed_limit), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, .flags = D },
    { "low_speed_time", "low speed time", OFFSET(low_speed_time), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, .flags = D },
    { "connect_timeout", "connect timeout", OFFSET(connect_timeout), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, .flags = D },
    { "read_async", "if read async", OFFSET(readasync), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1, D },
    { "load_file", "if load to file", OFFSET(loadfile), AV_OPT_TYPE_INT, { .i64 = 1 }, 0, 1, D },
    { "redapplication", "RedApplicationContext", OFFSET(app_ctx_intptr), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, .flags = D },
    { "http_proxy", "set HTTP proxy to tunnel through", OFFSET(http_proxy), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "referer", "override referer header", OFFSET(referer), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D },
    { "dns_timeout", "dns timeout", OFFSET(dns_timeout), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, INT_MAX, D },
    { "enable-url-list-fallback", "enable using the url list to retry network error", OFFSET(enable_url_list_fallback), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1, D },
    { "tcp_buffer", "socket option -- SO_RCVBUF (bytes)", OFFSET(tcp_buffer), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, INT_MAX, D },
    { "use_https", "replace http with https", OFFSET(use_https), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1, D },
