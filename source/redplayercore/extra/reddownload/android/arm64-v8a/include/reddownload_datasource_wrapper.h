...
typedef struct DownLoadOptWrapper_ {
  ...
  int dns_timeout;
  const char
      *url_list_separator; // if empty, not support url list; if not empty, url
                           // is a list splitted by a separator
  int use_https;
} DownLoadOptWrapper;
...
uint64_t reddownload_get_first_caton_offset_wrepper(const char *url);
